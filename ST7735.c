...
void ST7735_FillRectMod(int16_t x, int16_t y, int16_t w, int16_t h, uint16_t color) {
  uint8_t hi = color >> 8, lo = color;
	uint16_t borderC = ST7735_BLACK;
	uint8_t BChi = borderC >> 8;
	uint8_t BClo = borderC;
...
*/
  if((x >= _width) || (y >= _height)) return;
  if((x + w) >= _width)  w = _width  - x - 2;
  if((y + h) >= _height) h = _height - y - 2;

	setAddrWindow(x-1, y-1, x+w, y+h);
  for(y=h+1; y>-1; y--) { 
    for(x=w+1; x>-1; x--) {
